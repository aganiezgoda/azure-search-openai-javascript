...
const SYSTEM_MESSAGE_CHAT_CONVERSATION = `Assistant helps the Polish citizens with support questions regarding the Polish law and regulations. Be brief in your answers.
...
const FOLLOW_UP_QUESTIONS_PROMPT_CONTENT = `Generate 3 very brief follow-up questions that the citizen would likely ask next.
...
const QUERY_PROMPT_TEMPLATE = `Below is a history of the conversation so far, and a new question asked by the user that needs to be answered by searching in a knowledge base about the Polish law and regulations.
Generate a search query based on the conversation and the new question.
Do not include cited source filenames and document names e.g info.txt or doc.pdf in the search query terms.
Do not include any text inside [] or <<>> in the search query terms.
Do not include any special characters like '+' or '*'.
